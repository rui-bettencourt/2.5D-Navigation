...
class RobotMeshState(object):
    def __init__(self, robot_kinematics):
        try:
            ...
            # exit()
            with open("/home/rui/mesh_nav_ws/src/REMANI-Planner/remani_planner/mm_config/meshes/ur5/ur5.urdf", 'r') as urdf_file:
                self.robot = URDF.from_xml_string(urdf_file.read())

        #########Configs
        joint_states_topic = "/mm/mani/joint_state"
        pose_odom_topic = "/mm/car/odom"
        joints = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6']#, 'torso_lift_joint']
        self.not_body = [ "mani_1", "mani_2","mani_3", "mani_4", "mani_5"]
        self.body = ["mm_base", "mani_0"]
        robot_base_link = 'mm_base'
    ...
    #         #     continue
    #         if not self.only_body_mesh or (self.only_body_mesh and link.name not in self.not_body):
    #             try:
    ...
    def transform_whole_robot(self, bbs, positions):
        ...
        for bb_name in bbs.keys():
            bb = self.transform_obb(bbs[bb_name], transformation_matrix, bbs['mm_base'].center)
            bbs[bb_name] = bb
        return bbs
    ...
    def simulate_move_joints(self, bbs_og, joint_names, positions):
        bbs = (deepcopy(bbs_og))
        if len(joint_names) != len(positions) and joint_names != 'mm_base':
        ...
        if joint_names == 'mm_base':# and len(positions) in [3, 4]:
...
if __name__ == '__main__':
    ...
    print(mc.visual_origin_T)
    meshes = mc.update_robot_arm_bbs(pose, move_base = True, local_frame = False)
    ...
    # mc.robot_mesh_bb.paint_uniform_color([1.0,0.0,0.0])
    # o3d.visualization.draw_geometries([mc.robot_mesh_bb])
    # mc.save_mesh(path+'robotfastarmer_no_arms.ply')
    print("done")
    # rospy.spin()